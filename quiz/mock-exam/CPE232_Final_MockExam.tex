...
\item งานวิเคราะห์ข้อมูลในข้อใดต่อไปนี้ที่ \textbf{ไม่เหมาะสม} กับการประยุกต์ใช้แบบจำลอง Unsupervised Learning ประเภท Clustering?
\begin{enumerate}[label=\alph*.]
    \item การแบ่งกลุ่มลูกค้าที่มีพฤติกรรมการซื้อคล้ายคลึงกัน (Customer Segmentation)
    \item การสร้างกลุ่มของบทความที่มีเนื้อหาใกล้เคียงกันสำหรับการจัดหมวดหมู่
    \item การทำนายแนวโน้มยอดขายในเดือนหน้าจากข้อมูลในอดีต (Sales Forecasting)
    \item การตรวจหาลูกค้าหรือธุรกรรมที่มีพฤติกรรมแตกต่างไปจากกลุ่มหลัก (Anomaly/Outlier Detection)
\end{enumerate}

\needspace{7\baselineskip}
\item เมทริกซ์ประเมินผล (Evaluation Metric) ในข้อใดต่อไปนี้ ที่มักถูกนำมาใช้ประเมินโมเดลประเภท Classification แต่ \textbf{ไม่สามารถ} นำมาใช้วัดประสิทธิภาพของแบบจำลอง Regression ได้?
\begin{enumerate}[label=\alph*.]
...
\needspace{7\baselineskip}
\item แบบจำลองหรืออัลกอริทึมใดต่อไปนี้ที่ต้องพึ่งพาการคำนวณระยะห่าง (Distance Metric) เพื่อประเมินความเหมือน (Similarity) ของข้อมูลเป็นกลไกหลักในการทำนาย?
\begin{enumerate}[label=\alph*.]
...
\needspace{7\baselineskip}
\item สมการเชิงเส้นแบบพหุคูณ (Multiple Linear Regression) ในข้อใด \textbf{มีการรวมตัวแปรปฏิสัมพันธ์ (Interaction Term)} เข้าไปในแบบจำลอง?
\begin{enumerate}[label=\alph*.]
    \item $\hat{y} = \beta_0 + \beta_1 x_1$
    \item $\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_2$
    \item $\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3(x_1 \times x_2)$
    \item $\hat{y} = \beta_0 x_1 + \beta_1 x_2 + \epsilon$
\end{enumerate}

\needspace{7\baselineskip}
\item ข้อความใดต่อไปนี้ที่ \textbf{ไม่ถูกต้อง} และเป็นการอธิบายกลไกที่ผิดสำหรับ Agglomerative Hierarchical Clustering?
\begin{enumerate}[label=\alph*.]
    \item แผนภาพ Dendrogram จะแสดงให้เห็นถึงความใกล้เคียงและการรวมกลุ่มของข้อมูลในแต่ละขั้นตอน
    \item ในจุดเริ่มต้นของการทำงาน ข้อมูลแต่ละตัวแปร (Data point) จะถูกพิจารณาให้เป็นหนึ่ง Cluster แยกกัน
    \item เราสามารถระบุจำนวน Cluster สุดท้ายได้โดยการลากเส้นตัด Dendrogram แนวนอนที่ระดับความสูงใดๆ ที่เหมาะสม
    \item ข้อมูลแต่ละตัวจะถูกบังคับอัปเดตให้อยู่ในกลุ่มตามค่าเฉลี่ยใหม่ (Centroid) ของแต่ละรอบการทำงาน (Iteration) อย่างต่อเนื่อง
\end{enumerate}
...
\item ข้อใดต่อไปนี้จัดเป็น \textbf{Hyperparameter} (พารามิเตอร์ที่ต้องกำหนดล่วงหน้าก่อนกระบวนการ Training เริ่มขึ้น)?
\begin{enumerate}[label=\alph*.]
    \item ค่า \texttt{max\_depth} ของแบบจำลอง Decision Tree
    \item ค่าสัมประสิทธิ์ (Weights) ที่โมเดลเรียนรู้และปรับปรุงระหว่างกระบวนการ Gradient Descent
    \item อัตราการเรียนรู้ (Learning Rate, $\alpha$) ในสมการ Gradient Descent
    \item จำนวนคลัสเตอร์ $k$ (หรือ \texttt{n\_clusters}) สำหรับอัลกอริทึม K-Means
    \item ค่าจุดตัดแกน (Bias Term) ที่ได้หลังจากการดำเนินการ Optimization ของ Loss function
\end{enumerate}

\needspace{8\baselineskip}
\item แนวทางหรือเทคนิคในข้อใดบ้างที่มีส่วนช่วยวิศวกรข้อมูลในการ \textbf{ลดปัญหา Overfitting} ได้?
\begin{enumerate}[label=\alph*.]
    \item การเพิ่มปริมาณของข้อมูล (Training Data) ให้มีจำนวนมากและครอบคลุมยิ่งขึ้น
    \item การประยุกต์ใช้เทคนิค L1/L2 Regularization เพื่อทำโทษ weight ที่สูงเกินไป
    \item การลดค่า \texttt{max\_depth} เพื่อป้องกันไม่ให้ Decision Tree ซับซ้อนเกินไป
    \item การเพิ่มจำนวนฟีเจอร์ (Features) เข้าไปใน Dataset เพื่อเพิ่มรายละเอียดให้แบบจำลอง
    \item การเพิ่ม Learning Rate เป็นสองเท่าให้ Gradient Descent ลู่เข้าสู่คำตอบเร็วขึ้น
\end{enumerate}

\needspace{8\baselineskip}
\item ข้อใดบ้างเป็นข้อความที่ \textbf{อธิบายได้ถูกต้อง} เกี่ยวข้องกับ Silhouette Score สำหรับประเมินโหมด Clustering?
\begin{enumerate}[label=\alph*.]
    \item ค่านี้จะตกอยู่ในช่วงตั้งแต่ $[-1, +1]$ เสมอ
    \item หากค่าของจุดข้อมูลใกล้ $+1$ แสดงว่าข้อมูลนั้นอยู่ถูกคลัสเตอร์และแยกออกจากกลุ่มอื่นชัดเจน
    \item หากค่าประเมินติดลบ (Negative value) หมายความว่าจุดข้อมูลดังกล่าวอาจถูกจัด (Assign) กลุ่มผิด
    \item หากค่าอยู่ใกล้ $0$ แสดงว่าโมเดลแบ่ง Cluster ได้สมบูรณ์แบบที่สุด (Perfect clusters)
    \item สามารถนำมาใช้ประเมินผลได้เฉพาะอัลกอริทึม K-Means เท่านั้น
\end{enumerate}
...
\begin{enumerate}[label=\textbf{\arabic*.},start=9]
    \item \tf{} \quad ค่า Entropy = 0 หมายความว่า Node นั้น \textbf{ไม่มีความบริสุทธิ์} (Completely Impure) เลย
    \vspace{0.3cm}
    \item \tf{} \quad ค่า $R^2$ (Coefficient of Determination) สำหรับโมเดล Regression ในทางปฏิบัติอาจให้ผลลัพธ์ติดลบได้เช่นกัน หากทำนายสู้ค่าเฉลี่ยไม่ได้
    \vspace{0.3cm}
    \item \tf{} \quad อัลกอริทึม K-Means การันตีการลู่เข้า (Converge) สู่จุด Global Optimum เสมอ ไม่ว่าจะสุ่มจุดตั้งต้น (Initial Centroids) แบบใดก็ตาม
    \vspace{0.3cm}
    \item \tf{} \quad Stratified k-Fold Cross-Validation จะช่วยรักษาสัดส่วน (Ratio) ของ Target class ในข้อมูลสำหรับแต่ละ Fold เอาไว้ให้ใกล้เคียงกับจำนวนเต็มใน Dataset เดิม
    \vspace{0.3cm}
\end{enumerate}
...
\begin{itemize}
    \item \textbf{ข้อ 6:} \ans{a, c, d} (max\_depth, Learning Rate, n\_clusters คือ Hyperparameters; b,e คือ learned parameters)
      \item \textbf{ข้อ 7:} \ans{a, b, c} (การเพิ่มข้อมูล, L1/L2 Regularization, และการลด max\_depth ช่วยลด Overfitting ได้; ส่วน d เพิ่ม features เสี่ยงทำ curse of dimensionality; e เพิ่ม LR เสี่ยง diverge)

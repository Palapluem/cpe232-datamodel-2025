...
\textbf{โครงสร้างคะแนน:} Part 1 (10) \;|\; Part 2 (10) \;|\; Part 3 (15) \;|\; Part 4 (20) \;|\; Part 5 (45)
...
\item ข้อใดอธิบาย Naive Bayes Classifier ได้ถูกต้องที่สุด?
\begin{enumerate}[label=\alph*.]
    \item สร้างต้นไม้ตัดสินใจโดยใช้ Information Gain เลือก attribute ที่ดีที่สุด
    \item ใช้ทฤษฎีของเบย์และสมมติว่า features ทุกตัวมีความเป็นอิสระต่อกัน
    \item หาจุดที่ใกล้ที่สุด k จุดในพื้นที่ feature แล้ว vote ผลลัพธ์
    \item สร้าง Hyperplane เพื่อแบ่งกลุ่มข้อมูลให้มี margin กว้างที่สุด
\end{enumerate}

\needspace{7\baselineskip}
\item เหตุใด MSE จึงลงโทษ error ขนาดใหญ่หนักกว่า MAE?
\begin{enumerate}[label=\alph*.]
    \item MSE ยก error เป็นกำลังสองทำให้ค่า error ที่มากถูก amplify มากกว่า
    \item MSE คำนวณค่า median แทนค่าเฉลี่ยจึงไม่ไว้วางใจ outlier
    \item MSE หารด้วย $\sqrt{n}$ ส่วน MAE หารด้วย $n$
    \item MSE ใช้ log scale แทน linear scale ในการวัด error
\end{enumerate}

\needspace{7\baselineskip}
\item ข้อใดเป็นข้อดีหลักของ k-Fold Cross-Validation เมื่อเทียบกับ Single Train/Test Split?
\begin{enumerate}[label=\alph*.]
    \item ใช้เวลา training น้อยกว่าเพราะ dataset เล็กลง
    \item ลด variance ของการประเมิน model เพราะใช้ข้อมูลทุกส่วนเป็นทั้ง train และ validation
    \item ไม่จำเป็นต้องสุ่มแบ่งข้อมูลก่อน
    \item รับประกันว่า model จะไม่ overfit กับ training data
\end{enumerate}

\needspace{7\baselineskip}
\item DBSCAN มีข้อได้เปรียบเหนือ K-Means อย่างไร?
\begin{enumerate}[label=\alph*.]
    \item ทำงานเร็วกว่าบน dataset ขนาดใหญ่เสมอ
    \item ไม่ต้องระบุจำนวน cluster ล่วงหน้า และจัดการ noise/outlier ได้
    \item Cluster ที่ได้มี centroid ชัดเจนทุกครั้ง
    \item ต้องการ memory น้อยกว่าเพราะไม่เก็บ proximity matrix
\end{enumerate}

\needspace{7\baselineskip}
\item Model มี Training Accuracy = 97\%, Test Accuracy = 62\% สถานการณ์นี้คืออะไร?
\begin{enumerate}[label=\alph*.]
    \item Overfitting
    \item Underfitting
    \item Good Generalization
    \item High Bias, High Variance พร้อมกัน
\end{enumerate}
...
\begin{enumerate}[label=\alph*.]
    ...
    \item จำนวนต้นไม้ใน Random Forest (\texttt{n\_estimators})
...
\begin{enumerate}[label=\textbf{\arabic*.}]
    \item \textbf{Entropy (เอนโทรปี)} \hfill [Lecture 6]
    \vspace{2.2cm}

    \item \textbf{Information Gain} \hfill [Lecture 6]
    \vspace{2.2cm}
    ...
    \item \textbf{Gradient Descent} \hfill [Lecture 7]
    \vspace{2.2cm}
    ...
    \item \textbf{Centroid} \hfill [Lecture 9]
    \vspace{2.2cm}

    \item \textbf{Silhouette Score} \hfill [Lecture 9]
    \vspace{2.2cm}

    \item \textbf{DBSCAN (Density-Based Spatial Clustering of Applications with Noise)} \hfill [Lecture 9]
    \vspace{2.2cm}
\end{enumerate}
...
\needspace{8\baselineskip}
\item \textbf{[Lecture 6]} เพราะเหตุใดจึงควรเลือกใช้ \textbf{Random Forest} แทน \textbf{Decision Tree} เดี่ยว ในกรณีที่ต้องการ model ที่ generalize ดี? อธิบายกลไกที่ทำให้ Random Forest ดีกว่า
\vspace{3.5cm}
...
\needspace{8\baselineskip}
\item \textbf{[Lecture 9]} ผลการ cluster ด้วย K-Means ได้ Silhouette Score = 0.12 ค่านี้บอกอะไร? และคุณจะใช้วิธีใดในการ diagnose ว่าควรใช้กี่ cluster ที่เหมาะสม?
\vspace{3.5cm}

\needspace{8\baselineskip}
\item \textbf{[Lecture 8]} อธิบายปรากฏการณ์ ``Local Minimum'' ใน Gradient Descent ว่าเกิดขึ้นได้อย่างไร และ optimizer สมัยใหม่แก้ปัญหานี้อย่างไร (ยก optimizer อย่างน้อย 1 ตัวพร้อมอธิบาย)
\vspace{3.5cm}
...
\textbf{จงเปรียบเทียบ 4 Classification Algorithms ต่อไปนี้:}
\begin{itemize}
    \item Decision Tree
...
\textbf{การเปรียบเทียบต้องครอบคลุม:}
\begin{enumerate}
    \item หลักการทำงานโดยย่อ (1--2 ประโยค/algorithm)
    \item ข้อดีและข้อจำกัดหลัก
    \item ความเหมาะสมกับโจทย์นี้ โดยเฉพาะเรื่อง class imbalance และ dataset ขนาดใหญ่
    \item สรุปว่าควรเลือก algorithm ใด พร้อมเหตุผล
\end{enumerate}
...
% ─── Part 5 ─────────────────────────────────────────────────────────────────
\section*{Part 5: Calculation (45 คะแนน)}
\textbf{คำชี้แจง:} แสดงวิธีทำให้ครบทุกขั้นตอน คำตอบที่ไม่มีวิธีทำจะไม่ได้คะแนน

\vspace{0.3cm}
\subsection*{ข้อ 5.1 — Decision Tree: Entropy \& Information Gain (10 คะแนน)}

\textbf{กำหนดให้:} $\log_2(3) \approx 1.585$, \quad $\log_2(5) \approx 2.322$, \quad $\log_2(7) \approx 2.807$

ข้อมูลชุดหนึ่งสำหรับ predict ว่าลูกค้าจะ \textbf{``ซื้อสินค้า (ใช่/ไม่ใช่)''} มีทั้งหมด 8 samples ดังนี้:
...
\begin{tabular}{|c|c|c|c|c|}
    \hline
    \rowcolor{cyan!15}
    \textbf{\#} & \textbf{รายได้ (Income)} & \textbf{นักศึกษา (Student)} & \textbf{อายุ (Age)} & \textbf{ซื้อ (Buy)} \\
    \hline
    1 & สูง & ใช่ & น้อยกว่า 30 & ใช่ \\
    2 & สูง & ใช่ & น้อยกว่า 30 & ใช่ \\
    3 & สูง & ไม่ใช่ & 30--50 & ใช่ \\
    4 & ปานกลาง & ใช่ & 30--50 & ใช่ \\
    5 & ปานกลาง & ไม่ใช่ & น้อยกว่า 30 & ไม่ใช่ \\
    6 & ต่ำ & ไม่ใช่ & น้อยกว่า 30 & ไม่ใช่ \\
    7 & ต่ำ & ใช่ & 30--50 & ไม่ใช่ \\
    8 & ต่ำ & ไม่ใช่ & 30--50 & ไม่ใช่ \\
    \hline
\end{tabular}
\end{center}

\textbf{5.1.1} (2 คะแนน) คำนวณ $\text{Entropy}(S)$ ของ dataset ทั้งหมด
\vspace{2.5cm}

\textbf{5.1.2} (6 คะแนน) คำนวณ $\text{Information Gain}$ ของทุก attribute ได้แก่ \textbf{รายได้, นักศึกษา, อายุ} แสดงวิธีคิดทุกขั้น
\vspace{7.5cm}

\textbf{5.1.3} (2 คะแนน) ควรเลือก attribute ใดเป็น \textbf{Root Node} ของ Decision Tree? อธิบายเหตุผล
\vspace{2cm}

\newpage

\subsection*{ข้อ 5.2 — Confusion Matrix Metrics (8 คะแนน)}
...
\textbf{5.2.1} (5 คะแนน) คำนวณค่าต่อไปนี้พร้อมแสดงวิธีคิด:
\begin{itemize}
    \item Accuracy
    \item Precision
...
\textbf{5.2.2} (3 คะแนน) จากผลลัพธ์ข้างต้น จงตีความ: ระบบนี้เหมาะสมสำหรับการวินิจฉัยโรคหรือไม่? metric ใดสำคัญที่สุดในบริบทนี้? และควรปรับปรุงอย่างไร?
\vspace{3.5cm}

\newpage

\subsection*{ข้อ 5.3 — Regression: MSE, RMSE, MAE, R² และ Gradient Descent (10 คะแนน)}
...
\textbf{5.3.1} (7 คะแนน) คำนวณค่าต่อไปนี้พร้อมแสดงวิธีคิดทุกขั้น:
\begin{itemize}
...
\textbf{5.3.2} (3 คะแนน) กำหนดให้ model มี parameter $\theta_0 = 1.0$, $\theta_1 = 0.5$ และ Learning Rate $\alpha = 0.1$\\
gradient ที่คำนวณได้: $\frac{\partial L}{\partial \theta_0} = 2.0$, \quad $\frac{\partial L}{\partial \theta_1} = -1.5$\\
จงคำนวณค่า $\theta_0$ และ $\theta_1$ ใหม่หลัง Gradient Descent update \textbf{1 ครั้ง}
\vspace{3.5cm}

\newpage

\subsection*{ข้อ 5.4 — K-Means Clustering (5 คะแนน)}

กำหนดจุดข้อมูล 2D ดังนี้: A(1,1), B(2,3), C(3,2), D(6,5), E(7,6), F(8,4)\\
\textbf{Initial Centroids:} $\mu_1 = (1,1)$, $\mu_2 = (8,4)$

\textbf{5.4.1} (3 คะแนน) ทำ K-Means Iteration ที่ 1:
\begin{itemize}
    \item คำนวณ Euclidean Distance ของแต่ละจุดไปยัง $\mu_1$ และ $\mu_2$
    \item จัดกลุ่มแต่ละจุดเข้า cluster ที่ใกล้ที่สุด
    \item คำนวณ centroid ใหม่
\end{itemize}
\vspace{5.5cm}

\textbf{5.4.2} (2 คะแนน) ทำ Iteration ที่ 2 โดยใช้ centroid ใหม่จาก iteration ที่ 1 algorithm converge หรือยัง? อธิบาย
\vspace{4cm}

\newpage

\subsection*{ข้อ 5.5 — Hierarchical Agglomerative Clustering + Dendrogram + Nested Clusters (12 คะแนน)}
...
\textbf{5.5.1} (5 คะแนน) แสดงขั้นตอน Agglomerative Clustering ทีละ step จนเหลือ 1 cluster:\\
ระบุ: (1) คู่ที่ merge, (2) ระยะทางที่ merge, (3) Proximity Matrix ที่อัปเดตแล้ว ทุก iteration
\vspace{8cm}

\textbf{5.5.2} (4 คะแนน) วาด \textbf{Dendrogram} โดยระบุแกน Y เป็นระยะทาง (distance) และแสดงระดับที่ merge แต่ละครั้งให้ชัดเจน
\vspace{7cm}

\textbf{5.5.3} (2 คะแนน) วาด \textbf{Nested Cluster Diagram} แสดงการ group จุดข้อมูลในแต่ละ level
\vspace{5cm}

\textbf{5.5.4} (1 คะแนน) ถ้าตัด Dendrogram ที่ระดับ $d = 4$ จะได้กี่ cluster และประกอบด้วยจุดใดบ้าง?
\vspace{1.5cm}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
    \item \textbf{Random Forest vs Decision Tree:} Random Forest ใช้ Bagging สร้าง Decision Trees หลายต้นจาก bootstrap samples ต่างกัน แต่ละต้น vote ผลลัพธ์ Ensemble หลายต้นลด variance ทำให้ model stable กว่า DT เดี่ยวที่ overfit ง่าย นอกจากนี้ Random Forest ยังสุ่มเลือก features แต่ละ split (feature subsampling) ลด correlation ระหว่างต้นไม้
    ...
    \item \textbf{CM Interpretation:} TP=20, FN=80 หมายความว่า Recall = 20/100 = 0.20 ต่ำมาก --- model ``พลาด'' ผู้ป่วยจริง 80\% ถือว่า\textbf{อันตรายมาก} ในบริบทการแพทย์ ควรปรับ decision threshold ให้ต่ำลง หรือ oversample class บวก (SMOTE) เพื่อให้ model ตรวจจับ positive case ได้ดีขึ้น
...
\subsubsection*{ข้อ 1: เปรียบเทียบ 4 Models}

\begin{center}
\renewcommand{\arraystretch}{1.25}
\small
\begin{tabular}{|p{2.5cm}|p{3.8cm}|p{3.8cm}|p{3.5cm}|}
    \hline
    \rowcolor{cyan!15}
    \textbf{Model} & \textbf{หลักการ} & \textbf{ข้อดี / ข้อจำกัด} & \textbf{FinTech Use Case} \\
    \hline
    ...
    \hline
    KNN & วัดระยะทางหา k เพื่อนบ้านแล้ว vote & ง่าย / ช้ามากกับ 50K samples; ไม่ดีกับ imbalanced & \textbf{ไม่เหมาะ} (dataset ใหญ่, class imbalance) \\
    \hline
    Naive Bayes & P(class|features) ด้วย Bayes + independence assumption & เร็ว / independence มักไม่จริง & ใช้ได้เบื้องต้น แต่ features อาจ correlate กัน \\
    \hline
    Random Forest & Ensemble DT หลายต้น + Bagging + Feature sampling & Robust, ลด overfit / ตีความยากกว่า & \textbf{เหมาะที่สุด}: จัดการ imbalance ได้ดี, ไม่ overfit, scale ได้ \\
    \hline
\end{tabular}
\end{center}

\textbf{สรุป:} ควรเลือก \textbf{Random Forest} เนื่องจาก: (1) ทน imbalanced class ได้ดีกว่าเมื่อใช้ class\_weight (2) generalize ดีกว่า DT เดี่ยว (3) scale ได้กับ 50K samples (4) feature importance ช่วยตีความได้
...
\begin{itemize}
    ...
    \item \textbf{Naive Bayes:} สมมติ features อิสระกัน ทำงานเร็วมาก เหมาะกับ text classification
    \item \textbf{Random Forest = Bagging + Feature Subsampling} ลด variance ดีกว่า DT เดี่ยว feature importance ช่วยตีความ
